...
- Look through the sources and replace `openneuropet` with
  `<newname>` (hint: `git grep openneuropet` should find all
  spots).
- Delete the example command implementation in `datalad_openneuropet/hello_cmd.py`.
- Implement a new command, and adjust the `command_suite` in
  `datalad_openneuropet/__init__.py` to point to it.
- Replace `hello_cmd` with the name of the new command in
  `datalad_openneuropet/tests/test_register.py` to automatically test whether the
